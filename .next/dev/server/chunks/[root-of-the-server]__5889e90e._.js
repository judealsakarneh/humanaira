    ...
    try {
        ...
        if (!buyer_id || !seller_id) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'buyer_id and seller_id required'
            }, {
                status: 400
            });
        }
        ...
        if (gig_id && typeof gig_id === 'string' && gig_id.length < 36) {
            const { data: gigLookup, error: slugErr } = await supabase.from('gigs').select('id').eq('slug', gig_id).maybeSingle();
        ...
        let check = supabase.from('conversations').select('id').eq('seller_id', seller_id).eq('buyer_id', buyer_id);
        ...
        const { data: created, error: insertErr } = await supabase.from('conversations').insert([
            newRow
        ]).select('id').single();
        if (insertErr) {
            console.error('Insert error:', insertErr);
            // 🪄 Try finding again in case of race condition
            const { data: after } = await supabase.from('conversations').select('id').eq('seller_id', seller_id).eq('buyer_id', buyer_id).limit(1).maybeSingle();
            if (after) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    id: after.id
                }, {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5889e90e._.js.map